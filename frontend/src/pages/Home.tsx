...
function Home() {
  ...
  const [stats, setStats] = useState<DayAppointmentsStats>({
    pending: 0,
    taken: 0,
    skipped: 0,
    total: 0,
  ...
  const [familyMembers, setFamilyMembers] = useState<FamilyMemberIcon[]>([{ id: 0, name: 'Я', icon: '👤' }])
  ...
  const loadFamilyMembers = async () => {
    try {
      const response = await api.get<FamilyMemberApi[]>('/users/me/family')
  ...
  const loadAppointments = async (date: Date) => {
    ...
    try {
      ...
      if (
        familyMemberIdNum !== 0 &&
        familyMemberIdNum !== null &&
        familyMemberIdNum !== undefined &&
        !isNaN(familyMemberIdNum)
      ) {
      ...
      const data = (await response.json()) as DayAppointmentsResponse
                    ...
                    <div className="appointment-status-controls">
                      <button
                        ...
                        onClick={(e) => {
                          ...
                          void handleStatusChange(appointment.id, 'taken')
                        }}
                        title="Принял"
                      >
                        ✅
                      ...
                      <button
                        ...
                        onClick={(e) => {
                          ...
                          e.stopPropagation()
                          e.currentTarget.blur()
                          void handleStatusChange(appointment.id, 'skipped')
                        }}
                        title="Не принял"
                      >
                        ❌
                      ...
                      <button
                        ...
                        onClick={(e) => {
                          ...
                          e.stopPropagation()
                          e.currentTarget.blur()
                          void handleStatusChange(appointment.id, 'pending')
                        }}
                        title="Предстоит"
                      >
                        ⚪
